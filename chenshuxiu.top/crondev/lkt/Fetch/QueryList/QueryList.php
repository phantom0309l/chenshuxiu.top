...
class QueryList
{
     ...
     * @return mixed
     */
    public static function Query($page, array $rules, $range = '', $proxy = [], $cookie = null, $outputEncoding = null, $inputEncoding = null, $removeHead = false) {
        return self::getInstance()->_query($page, $rules, $range, $proxy, $cookie, $outputEncoding, $inputEncoding, $removeHead);
    }
     ...
     * @return QueryList
     */
    public function setQuery(array $rules, $range, $proxy = [], $cookie = null, $outputEncoding = null, $inputEncoding = null, $removeHead = false) {
        return $this->_query($this->html, $rules, $range, $proxy, $cookie, $outputEncoding, $inputEncoding, $removeHead);
    }

    private function _query($page, array $rules, $range, $proxy = [], $cookie = null, $outputEncoding, $inputEncoding, $removeHead) {
        $this->proxy = $proxy;
        $this->cookie = $cookie;
        $this->data = array();
...
//            trigger_error("The received content is empty!",E_USER_NOTICE);
    ...
    private function _request($url) {
        if (function_exists('curl_init')) {
            ...
            if (!empty($this->proxy)) {
                echo "\n---------------使用代理---------------\n";
                $proxy_ip = $this->proxy[0];
                $proxy_port = $this->proxy[1];
...
//curl_setopt($ch, CURLOPT_PROXYUSERPWD, ":"); //http代理认证帐号，username:password的格式
            ...
            $result = curl_exec($ch);
            $this->state = curl_getinfo($ch, CURLINFO_HTTP_CODE);
            curl_close($ch);
