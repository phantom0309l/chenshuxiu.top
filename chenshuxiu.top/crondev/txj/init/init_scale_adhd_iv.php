...
class Adhd_iv_initer
{
    ...
    public function getAnswersheetids () {
        return Dao::queryValues("select id from answersheets where qsheet_name='adhd_iv' and qsheet_ver in ('2.0')");
    }
    ...
    public function createXAnswers ($xanswersheet, $answersheet) {
        ...
        foreach ($xquestionsheet->getQuestions() as $q) {
            ...
            $row["pos"] = $pos;
            $row["content"] = $this->getXAnswerContent($pos);
    ...
    public function getXAnswerContent ($pos) {
        $posArr = array(
            0.1 => "注意",
            9.1 => "多动",
            14.1 => "冲动",
