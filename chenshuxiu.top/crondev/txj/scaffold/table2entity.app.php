...
createDaoFile($entityName, $filedirectory);

function createDaoFile ($entityName, $filedirectory) {
    if (! filterDao($entityName, $filedirectory)) {
        return;
    }
    $str = '<?php
/*
...
function filterDao ($entityName, $filedirectory) {
    $canCreate = true;
    $hostdir = ROOT_TOP_PATH . "/domain/{$filedirectory}/";
...
$allTables[$tableName] = table2array($tableName);
// }
...
foreach ($allTables as $tableName => $fields) {
    ...
    print_r($fields);
    array2entity($tableName, $fields, $entityName, $filedirectory);
}
...
// 将数组生成entity
function array2entity ($tableName, $fields, $entityName, $filedirectory) {
...
';
    ...
    // $entityName = $tableNames[$tableName];
    // $entityName = $argv[2];
    echo $entityName;
    ...
    // if ($ref == "ref") {
    // $entityName = substr ( $entityName, 0, strlen ( $entityName ) - 3 ) .
    // "Ref";
    // }
